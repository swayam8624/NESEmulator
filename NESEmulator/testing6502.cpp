...
class Demo_cpu6502 : public kairo::PixelGameEngine
{
public:
	...
	void DrawcpuWrite(int x, int y, uint16_t nAddr, int nRows, int nColumns)
	{
		int ncpuWriteX = x, ncpuWriteY = y;
		for (int row = 0; row < nRows; row++)
		{
			...
			DrawString(ncpuWriteX, ncpuWriteY, sOffset);
			ncpuWriteY += 10;
		}
	}
	...
	bool OnUserCreate()
	{
		// Load ProgcpuWrite (assembled at https://www.masswerk.at/6502/assembler.html)
		/*
			*=$8000
			LDX #10
			STX $0000
		...
		*/
		...
		// Convert hex string into bytes for cpuWrite
		std::stringstream ss;
		...
		while (!ss.eof())
		{
			std::string b;
			ss >> b;
			nes.cpuWrite(nOffset++, (uint8_t)std::stoul(b, nullptr, 16));
		}

		// Set Reset Vector
		nes.cpuWrite(0xFFFC, 0x00); // Use cpuWrite
	...
	bool OnUserUpdate(float fElapsedTime)
	{
		...
		if (GetKey(kairo::Key::SPACE).bPressed)
		{
			do
			{
				nes.cpu.clock();
		...
		if (GetKey(kairo::Key::R).bPressed)
			nes.cpu.reset();

		if (GetKey(kairo::Key::I).bPressed)
			nes.cpu.irq();

		if (GetKey(kairo::Key::N).bPressed)
			nes.cpu.nmi();

		// Draw cpuWrite Page 0x00		
		DrawcpuWrite(2, 2, 0x0000, 16, 16);
		DrawcpuWrite(2, 182, 0x8000, 16, 16);
		DrawCpu(448, 2);
		DrawCode(448, 72, 26);
